<?php
/**
 * Validation (Lite). Same logic as Pro.
 *
...
class DropLock_Validator {
	...
	public function validate_add_to_cart( $passed, $product_id, $quantity, $variation_id = 0, $variations = array() ) {
		if ( ! $passed || DropLock_Helper::current_user_can_bypass() ) {
			return $passed;
		}
		...
		$purchased      = DropLock_Order_Query::get_customer_purchased_quantity( $effective_id, $user_id, $billing_email, $statuses );
	...
	public function check_cart_items() {
		if ( DropLock_Helper::current_user_can_bypass() || ! WC()->cart ) {
			return;
		}
